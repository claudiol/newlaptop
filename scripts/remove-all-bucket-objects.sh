...
do
    ...
    if [ $CHECK -gt 0 ] && [ "$ans" != "A" ]; then
	  ...
	  read ans
	  if [ "$ans" == "y" ] || [ "$ans" == "Y" ]; then
	    log -n " $CHECK REMOVING ${BUCKETS[$i]} OBJECTS OLDER THAN 30 DAYS ... "
	    RC=$(aws s3 rm s3://${BUCKETS[$i]} --recursive > /dev/null 2>&1)
	    log " $CHECK REMOVING ${BUCKETS[$i]} OLDER THAN 30 DAYS ... DONE"
          fi
    elif [ $CHECK -gt 0 ]; then 
          log -n " $CHECK REMOVING ${BUCKETS[$i]} OBJECTS OLDER THAN 30 DAYS ... "
	  RC=$(aws s3 rm s3://${BUCKETS[$i]} --recursive > /dev/null 2>&1;echo $?)
	  if [ $RC -eq 0 ]; then
	    log " $CHECK REMOVING ${BUCKETS[$i]} OLDER THAN 30 DAYS ... DONE"
	  else
